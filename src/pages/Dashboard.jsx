...
import { mergePrefs, ALL_KPIS, WIDGET_LABELS } from '../lib/dashboardPrefs'
...
export default function Dashboard({ session, isGuest, profile, onUpgrade, onTabChange, initialShareParams }) {
  ...
  // Widget order
  const widgetOrder = premium ? (prefs.widgetOrder || ['health','kpis','framework','charts','insights','goals']) : ['health','kpis','framework','charts','insights','goals']

  const WIDGETS = {
    health: prefs.showHealthScore !== false && (
    ...
    framework: (prefs.showFramework !== false) && (
    ...
    insights: prefs.showInsights !== false && (
      <div key="insights">
        ...
        <div className={styles.insightsRow}>
          ...
          <div className={styles.insightCard}>
            ...
            <p>{savingsRate >= 20 ? `At ${savingsRate}%, you're above the recommended 20%. Excellent!` : `You're saving ${savingsRate}% of income. The goal is 20%.`}</p>
          </div>
        </div>
      </div>
    ),

    goals: prefs.showGoals !== false && (
      <div key="goals">
  ...
  // Health and KPIs are always shown together in the health row
  const healthAndKpis = (
  ...
  const orderedWidgets = widgetOrder.map(id => {
    if (id === 'health' || id === 'kpis') return null // handled together
  ...
  return (
    <div className={styles.dashboard}>
      <div className={styles.header}>
        <div>
          {prefs.showGreeting !== false && <h1>Dashboard</h1>}
          {!prefs.showGreeting && <h1>Dashboard</h1>}
          <p>Your financial snapshot for {formatMonthLabel(month)}</p>
        </div>
      ...
      {/* Always show health + KPIs together at top */}
      {healthAndKpis}

      {/* Ordered remaining widgets */}
      {orderedWidgets}
