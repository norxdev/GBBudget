...
function emptyRow(type) {
  return { id: crypto.randomUUID(), description: '', category: type === 'income' ? 'Employment' : type === 'expense' ? 'Housing' : 'Emergency', entry_type: type, frequency: 'recurring', amount: '', isNew: true }
}
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.summaryBar}>
        {[
          { label: 'Total Income', value: '$' + totalIncome.toLocaleString(), color: null },
          { label: 'Total Expenses', value: '$' + totalExpenses.toLocaleString(), color: 'var(--red)' },
          { label: 'Savings Allocated', value: '$' + totalSavings.toLocaleString(), color: null },
    ...
    <div className={styles.section}>
      ...
      <table className={styles.table}>
        ...
        <tbody>
          {rows.length === 0 && (
            <tr>
              <td colSpan={5} className={styles.emptyRow}>No entries yet — click "+ Add row" to start</td>
            </tr>
          )}
          {rows.map(row => (
            <tr key={row.id}>
