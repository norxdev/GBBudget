...
function calcHealthScore({ income, needs, wants, savings }) {
  ...
  return {
    ...
    wantsRatio: Math.round(wantsRatio * 100),
  }
}
...
export default function Dashboard({ session, onUpgrade, onTabChange }) {
  ...
  const catColors = catLabels.map(c => CAT_COLORS[c] || '#95A5A6')
  ...
  const barData = {
    ...
    datasets: [
      {
        label: 'Income',
        data: [6800, 7000, 7100, 7200, 7300, income || 7400],
      ...
      {
        label: 'Expenses',
        data: [5200, 5400, 5100, 5600, 5200, totalExpenses || 5640],
  ...
  return (
    <div className={styles.dashboard}>
      <div className={styles.header}>
        <div>
          <h1>Good morning 👋</h1>
          <p>Here's your financial snapshot for {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
        </div>
        <div className={styles.period}>📅 {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })} ▾</div>
      </div>
      ...
      {!hasData && (
        <div className={styles.emptyBanner}>
          <span>📊</span>
          <div>
            <strong>No budget data yet for this month.</strong>
      ...
      <div className={styles.healthRow}>
          ...
          <div className={styles.subScores}>
            {[
              ...
              <div key={label} className={styles.subScoreRow}>
                ...
                <div className={styles.subBar}><div className={styles.subFill} style={{ width: val + '%' }} /></div>
        ...
        <div className={styles.kpiGrid}>
          {[
            { icon: '💰', label: 'Monthly Income', value: '$' + income.toLocaleString(), change: null, color: '#D8EDE4' },
            { icon: '💸', label: 'Total Expenses', value: '$' + totalExpenses.toLocaleString(), change: null, color: '#FDEEE9' },
            { icon: '🏦', label: 'Monthly Savings', value: '$' + savings.toLocaleString(), change: null, color: '#D8EDE4' },
            { icon: '📊', label: 'Savings Rate', value: income > 0 ? Math.round(savings / income * 100) + '%' : '0%', change: null, color: '#FBF3DC' },
            { icon: '💵', label: 'Remaining', value: '$' + remaining.toLocaleString(), change: null, color: remaining >= 0 ? '#D8EDE4' : '#FDECEA' },
            { icon: '🎯', label: 'Active Goals', value: goals.length + ' goals', change: null, color: '#D6EAF8' },
          ].map(kpi => (
          ...
          {[
            ...
            <div key={fb.name} className={styles.fbItem}>
              ...
              <div className={styles.fbDesc}>
                {fb.actual <= fb.target
                  ? `✅ Within target — great discipline!`
                  : `⚠️ ${fb.actual - fb.target}% over target — room to improve`}
                {' '}{fb.desc}
              </div>
            </div>
          ))}
        ...
        <div className={styles.chartCard}>
          <div className={styles.chartHeader}>
            <div>
              ...
              <div className={styles.chartSubtitle}>This month's breakdown</div>
            </div>
          </div>
          {catData.length > 0
      ...
      <div className={styles.insightsRow}>
        {diningEntry && Number(diningEntry.amount) > 400 ? (
          ...
          <div className={styles.insightCard}>
            <div className={styles.insightType} style={{ color: 'var(--accent)' }}>
              <div className={styles.insightDot} style={{ background: 'var(--accent)' }} />
              Doing Well
            </div>
        ...
        {subTotal > 100 ? (
          <div className={styles.insightCard}>
            <div className={styles.insightType} style={{ color: 'var(--amber)' }}>
              <div className={styles.insightDot} style={{ background: 'var(--amber)' }} />
              Opportunity
            </div>
        ...
        <div className={styles.insightCard}>
          ...
          <p>
            {savingsRate >= 20
              ? `At ${savingsRate}%, you're saving above the recommended 20%. Excellent work!`
